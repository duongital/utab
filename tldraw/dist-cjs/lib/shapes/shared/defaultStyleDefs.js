...
function getFontDefForExport(fontStyle) {
  return {
    ...
    async getElement() {
      ...
      const fontFile = await (await (0, import_editor.fetch)(url)).blob();
      const base64FontFile = await import_editor.FileHelpers.blobToDataUrl(fontFile);
      const newFontFaceRule = fontFaceRule.replace(url, base64FontFile);
      return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("style", { children: newFontFaceRule });
    }
  };
}
